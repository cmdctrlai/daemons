...
import { CopilotSessionWatcher, CopilotSessionEvent, CopilotCompletionEvent } from '../session-watcher';
...
export class DaemonClient {
  ...
  private sessionWatcher: CopilotSessionWatcher;
  ...
  constructor(config: CmdCtrlConfig, credentials: Credentials) {
    ...
    this.adapter = new CopilotAdapter(this.sendEvent.bind(this));
    this.sessionWatcher = new CopilotSessionWatcher(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('unexpected-response', (_req, res) => {
        if (res.statusCode === 401) {
          ...
          console.error('Run "cmdctrl-copilot-cli register" again to re-register this device.');
  ...
  private handleSessionEvent(event: CopilotSessionEvent): void {
    this.send({
      type: 'event',
      task_id: '',
  ...
  private handleSessionCompletion(event: CopilotCompletionEvent): void {
