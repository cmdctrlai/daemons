...
export async function update(): Promise<void> {
  const packageName = '@cmdctrl/cursor-ide';
  ...
  // Verify the update
  try {
    const result = execSync(`cmdctrl-cursor-ide --version`, { encoding: 'utf-8' }).trim();
    console.log(`\nUpdated successfully to v${result}`);
  } catch {
    console.log(`\nUpdate installed. Run 'cmdctrl-cursor-ide --version' to verify.`);
  }

  // Restart daemon if it was running before update
  if (wasRunning) {
    console.log('Restarting daemon...');
    const child = spawn('cmdctrl-cursor-ide', ['start'], {
      detached: true,
      stdio: 'ignore',
    });
    child.unref();
