...
program
  .name('cmdctrl-gemini-cli')
  .description('CmdCtrl daemon for Google Gemini CLI')
  .version('0.1.0');
...
program
  ...
  .option('-n, --name <name>', 'Device name (defaults to hostname-gemini)')
  .action(register);
...
program
  .command('start')
  .description('Start the Gemini CLI daemon and connect to the CmdCtrl server')
