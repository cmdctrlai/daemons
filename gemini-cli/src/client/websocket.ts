...
import { GeminiSessionWatcher, GeminiSessionEvent, GeminiCompletionEvent } from '../session-watcher';
...
export class DaemonClient {
  ...
  private sessionWatcher: GeminiSessionWatcher;
  ...
  constructor(config: CmdCtrlConfig, credentials: Credentials) {
    ...
    this.adapter = new GeminiAdapter(this.sendEvent.bind(this));
    this.sessionWatcher = new GeminiSessionWatcher(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('unexpected-response', (_req, res) => {
        if (res.statusCode === 401) {
          ...
          console.error('Run "cmdctrl-gemini-cli register" again to re-register this device.');
  ...
  private handleSessionEvent(event: GeminiSessionEvent): void {
    this.send({
      type: 'event',
      task_id: '',
  ...
  private handleSessionCompletion(event: GeminiCompletionEvent): void {
