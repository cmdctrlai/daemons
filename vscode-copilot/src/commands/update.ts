...
export async function update(): Promise<void> {
  const packageName = '@cmdctrl/vscode-copilot';
  ...
  // Verify the update
  try {
    const result = execSync(`cmdctrl-vscode-copilot --version`, { encoding: 'utf-8' }).trim();
    console.log(`\nUpdated successfully to v${result}`);
  } catch {
    console.log(`\nUpdate installed. Run 'cmdctrl-vscode-copilot --version' to verify.`);
  }

  // Restart daemon if it was running before update
  if (wasRunning) {
    console.log('Restarting daemon...');
    const child = spawn('cmdctrl-vscode-copilot', ['start'], {
      detached: true,
      stdio: 'ignore',
    });
    child.unref();
