...
export async function start(options: StartOptions): Promise<void> {
  ...
  console.log(`Starting CmdCtrl VS Code Copilot daemon...`);
  console.log(`Server: ${config.serverUrl}`);
  console.log(`Device: ${config.deviceName} (${config.deviceId})`);

  // Check if VS Code storage exists
  if (!vscodeStorageExists()) {
    console.warn('\nWarning: VS Code workspace storage not found.');
    console.warn('Make sure VS Code has been run at least once.');
  }
  ...
  if (!cdpAvailable) {
    console.warn('\nWarning: VS Code CDP not available.');
