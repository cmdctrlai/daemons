...
export async function update(): Promise<void> {
  const packageName = '@cmdctrl/opencode';
  ...
  try {
    const result = execSync(`cmdctrl-opencode --version`, { encoding: 'utf-8' }).trim();
    console.log(`\nUpdated successfully to v${result}`);
  } catch {
    console.log(`\nUpdate installed. Run 'cmdctrl-opencode --version' to verify.`);
  }

  if (wasRunning) {
    console.log('Restarting daemon...');
    const child = spawn('cmdctrl-opencode', ['start'], {
      detached: true,
      stdio: 'ignore',
    });
    child.unref();
