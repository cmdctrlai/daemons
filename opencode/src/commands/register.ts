...
export async function register(options: RegisterOptions): Promise<void> {
  ...
  const deviceName = options.name || `${os.hostname()}-opencode`;

  if (configManager.isRegistered()) {
    const config = configManager.readConfig();
    console.log(`Already registered as "${config?.deviceName}" (${config?.deviceId})`);
    console.log(`Server: ${config?.serverUrl}`);
    console.log(`\nTo re-register, run: cmdctrl-opencode unregister`);
    return;
  }

  console.log(`Registering OpenCode device "${deviceName}" with ${serverUrl}...\n`);
  ...
  console.log(`\nRun 'cmdctrl-opencode start' to connect to the server.`);
}
