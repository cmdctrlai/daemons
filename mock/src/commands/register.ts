...
export async function register(options: RegisterOptions): Promise<void> {
  ...
  const deviceName = options.name || `${os.hostname()}-mock`;
  ...
  console.log(`Registering mock device "${deviceName}" with ${serverUrl}...\n`);
  ...
  try {
    const response = await request(`${serverUrl}/api/devices/code`, 'POST', {
      ...
      agentType: 'mock', // Use 'mock' as agent type
    });
  ...
  if (process.stdin.isTTY) {
    ...
    if (startNow) {
      const logFile = '/tmp/cmdctrl-daemon-mock.log';
